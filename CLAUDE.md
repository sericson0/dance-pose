...
The skeleton *and* muscle *views* are imported anatomical GLBs whose parts are re-parented onto our own joint rig (`src/skeletonMesh.js`); `main.js` loads them with top-level `await` before building the figures and falls back to procedural geometry on failure. The muscle GLB shares the skeleton's atlas frame, so it is loaded only when the skeleton did and bakes with the skeleton's scale. Both GLBs are CC-BY-SA, so distributing this project with them requires attribution + a compatible license (`public/models/ATTRIBUTION.md`).
